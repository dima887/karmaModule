<div class="col-xl-3 col-lg-4 col-md-5">
    <div class="sidebar-categories">
        <div class="head"><a href="{{route('category')}}" class="text-white">Категории</a></div>
        <ul class="main-categories">
            @foreach($category as $val)
                <li class="main-nav-list">
                    <a href="{{route('category', ['slug' => $val->slug])}}">{{$val->category}}
                    ...
                    @foreach($brand as $val)
                        <li class="filter-list">
                            <input class="pixel-radio" type="radio" id="{{$val->brand}}"
                                   name="brand">
                            <label for="apple">
                                <a href="{{route('category', ['slug' => $val->slug])}}" class="text-dark">{{$val->brand}}
